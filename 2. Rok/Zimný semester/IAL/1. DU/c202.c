...
void Stack_Init( Stack *stack ) {
	//Check if the stack is allocated
	if(stack == NULL){
		//If not, Error and return
		Stack_Error(SERR_INIT);
		return;
	}
	//If the stack is allocated, then set the top
	stack->topIndex = -1;
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	if(Stack_IsEmpty(stack)){
		//If yes, Error and return
		Stack_Error(SERR_TOP);
		return;
	}
	//If not, set value of dataPrt to that of the top
...
void Stack_Pop( Stack *stack ) {
	if(!Stack_IsEmpty(stack)){
		//If the stack is not empty, decrease the index of the top
		stack->topIndex--;
	}
}
...
void Stack_Push( Stack *stack, char data ) {
	//If the stack is full, ERROR and return
	if(Stack_IsFull(stack)){
		Stack_Error(SERR_PUSH);
		return;
	}
	//If not, increase the index of the top and input the data into the array
	stack->topIndex++;
